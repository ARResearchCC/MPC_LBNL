#=
This file contains utility functions that together help define the reduced order model in Julia.
=#
function passive_model(i, df_w, df_s, steps, stepsize)
    # Initialize empty DataFrames with the same columns as df_w
    Weather = DataFrame([Symbol(col) => Any[] for col in names(df_w)])
    # Initialize Schedules DataFrame with appropriate column names for the matrix
    Schedules = DataFrame([Symbol("col_$j") => Any[] for j in 1:size(df_s, 2)])

    start_index = i
    for step in steps
        next_index = start_index + Int(step / stepsize)
        if next_index <= nrow(df_w)
            push!(Weather, df_w[start_index, :])
            push!(Schedules, vec(df_s[start_index, :])')
    ...
    Weather.DayStatus = zeros(NumTime)
    ...
    for i = 1:NumTime
        ...
        Weather.DayStatus[i] = IsDay(Weather.SolarTime[i], Weather.datetime[i])  # [BIN]
        Weather.RadCool[i] = Q_radiativeCooling(T_indoor_constant, Weather.Temperature[i], Weather[i,:"Dew Point"], Weather.SolarTime[i], Weather.DayStatus[i]) # [BTU/hr]
        Weather.PV[i] = calculate_PV(Weather[i,:datetime], Weather[i,:DNI], Weather[i,:DHI], Weather[i,:GHI], 180-Berg_tilt, module_surface_tilt, Weather[i,:celltemp]) # [kW Generation / kW Capacity]
    end

    Weather.Lighting = Schedules[:,1];
    Weather.Plugs = Schedules[:,2];
    Weather.Occupancy = Schedules[:,3];
...
# Define a function to simulate an ambient temperature forecast by adding reasonable errors to the real temperature data
function Add_Uncertainty_Temp(actual_TS; alpha = 0.95, ma = false)
    l = length(actual_TS)
    temp_sim = similar(actual_TS, Float64)
    temp_sim[1] = actual_TS[1]
    delta = [i / l for i in 1:l]
    for i in 2:l
        incre = (actual_TS[i] - actual_TS[i-1]) * (1 + rand([-1, 1]) * delta[i])
        temp_sim[i] = alpha * temp_sim[i-1] + (1 - alpha) * actual_TS[i-1] + incre
    end
    if ma
        temp_sim = Moving_average(temp_sim, 3)
    end
    return temp_sim
end
...
function fahrenheit_to_kelvin(temp_f::Vector{Float64})::Vector{Float64}
