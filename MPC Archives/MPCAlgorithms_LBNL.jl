...
function OptimizeV4(iteration, δt, Weather, Schedules, M_States, J_States)
    ...
    begin
        ...
        if iteration > 1
            ...
            Total_Load_0 = E_Modelica_0 + (E_other_0)*δt # [kWh] Actual total Electrical Load at last timestep
            ...
            begin
                ...
                @constraint(m_0, InStorageBattery_past[2] == InStorageBattery_past[1] * (1 - δt * BatteryLoss) + δt * (PV2B_past * η - B2H_past)); # [kWh]
    ...
    begin
        ...
        @variable(m, PCM_SOC[1:2, 1:NumTime] >= 0); # PCM_SOC[1, t] ==> PCM Hot SOC; PCM_SOC[2, t] ==> PCM Cold SOC
    ...
    begin
        ...
        @objective(m, Min, δt * sum(G2H[t] for t = 1:NumTime)); # [kWh]
    end
    ############# Expressions ############
    begin
        # DeltaTemp 
    ...
    begin
        ...
        @constraint(m, [t=1:NumTime-1], InStorageBattery[t+1] == InStorageBattery[t] * (1 - δt * BatteryLoss) +  δt * (PV2B[t] * η - B2H[t])); # [kWh]
    ...
    CurrentCost = δt * value.(G2H[1]) # [kWh]
    return CurrentCost, J_States_new, [Modelica_Input, Modelica_Input]
end
