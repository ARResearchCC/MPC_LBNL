...
# Latest MPC, last changed 01/28/2025
function OptimizeV5_1(iteration, TC, steps, base_stepsize, input_df, M_States, J_States, predetermined_mode)
    ...
    begin
        ...
        T_HP_init = M_States[msize, :"Heat Pump Supply Water Temperature"] - 273.15 # [°C]
    ...
    # HP supply water temperature set point for each mode
    T_set = [20.0, 55.0, 8.0, 48.0, 11.0] # [°C]
    
    # PCM charging efficiency slope and intercept for each mode: PCM_efficiency = a_w * T_HP + b_w
    a_w = [0.0, 1/15, -1/12, 1/15, -1/12]  # Slopes for modes 1-5
    b_w = [1.0, -40/15, 20/12, -33/15, 23/12]  # Intercepts for modes 1-5

    begin
        # Define the model name and solver. In this case, model name is "m"
        begin
            m = Model(Gurobi.Optimizer)
            set_optimizer_attribute(m, "NonConvex", 2)  # Allow quadratic terms
            set_optimizer_attribute(m, "Method", 0)  # 2 corresponds to the barrier method
            set_optimizer_attribute(m, "FeasibilityTol", 1e-5)  # Allow slightly looser feasibility
            set_optimizer_attribute(m, "IntFeasTol", 1e-5)     # Default integrality tolerance
            set_optimizer_attribute(m, "OptimalityTol", 1e-3)  # Looser optimality tolerance (0.1%)
    ...
    begin
        ...
        @variable(m, T_HP[1:NumTime] >= 0)  # Heat pump supply temperature

        @variable(m, PCM_efficiency[1:NumTime])  # Efficiency of HP supply water charging PCMs
        #=
            (ON_Ratio[1, t]) ==> H2HP
            (ON_Ratio[2, t]) ==> H2C;
            (ON_Ratio[3, t]) ==> PCM_H2H;
    ...
    begin
        ...
        # @expression(m, Mode_switch_penalty[t=1:NumTime-1], sum( PCM_state[2, t] - PCM_state[2, t+1]  for ω = 2:5) * P_Pumps * δt[t]);

        @expression(m, PCM_charging_efficiency[t=1:NumTime], PCM_state[2, t]*PCM_efficiency[t] + PCM_state[3, t]*PCM_efficiency[t]);
        
        # Set single objective for minimizing loss of load in the current iteration of optimization horizon
        # minimizes loss of load cost (very high), while keeping the storages full while maximizing curtailment (free energy)
        @objective(m, Min, lossofloadcost*sum(G2H[t] for t = 1:NumTime) - 0.01*sum(PV2G[t] for t = 1:NumTime) - 50*sum(PCM_charging_efficiency[t] for t = 1:NumTime) + 
        0.1*sum(BatterySize - InStorageBattery[t] for t = 1:NumTime) + 10*sum((1.05-PCM_SOC[n, t]) for n = 1:2 for t = 1:NumTime)); # [kWh]
    end
    ############# Expressions ############
    begin
        # DeltaTemp 
    ...
    begin
        ...
        # Initialization of the HP supply water temperature
        @constraint(m, T_HP[1] == T_HP_init)

        # Temperature evolution constraint of the HP supply water temperature
        for t in 1:NumTime-1
            # The HP supply water temperature setpoint of the current mode (that it is on OR it is switching to)
            current_T_set = sum(PCM_state[w, t] * T_set[w] for w in 1:5)

            # Temperature evolution of the HP supply water temperature (linear transition over 3 steps to coverge to the targeted HP supply water temperature setpoint)
            @constraint(m, T_HP[t+1] == T_HP[t] + (current_T_set - T_HP[t])/3)
        end

        # PCM charging efficiency constraints (depends on the current HP supply water temperature and the HP supply water temperature setpoint of the active mode)
        for t in 1:NumTime
            @constraint(m, PCM_efficiency[t] == sum( (a_w[w] * T_HP[t] + b_w[w]) * PCM_state[w,t] for w in 1:5 ))
        end

        # Warm-start constraint to ensure continuity between iterations of the MPC optimization. The operation of the PCM of the first timestep of the current MPC iteration is always consistent with
        # the operation of the PCM of the second timestep of the last MPC iteration (what to do now is always predetermined last iteration)
        for w in 1:5
            @constraint(m, PCM_state[w, 1] == predetermined_mode[w])
        end
    ...
    begin
        if status == MOI.OPTIMAL
            ...
            # next action (for the warm-start constraint of the next iteration)
            next_state = value.(PCM_state[:,2])
            # Total penalty for loss of load
            CurrentCost = value.(G2H[1]) # [kWh]
            ...
            # J_States_new = [PVGeneration[1] * PVSize, (E_Lighting[1] + E_Plugs[1] + P_Controls + P_AHU), InStorageBattery_1, 0] # Default values for J_States
            Modelica_Input = 0              # Default value for Modelica_Input
            next_state = [1, 0, 0, 0, 0]    # Default next action (Stand by mode)
            CurrentCost = 0.0               # Default cost
        end
    end

    # Return results
    return CurrentCost, J_States_new, [Modelica_Input, Modelica_Input], Didnt_Solve, next_state
end